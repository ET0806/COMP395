...
public abstract class Animal : Food
{
    ...
    public GameObject babyPrefab; //Prefab from Hare (manually dragged in via the graphical interface)
    public Animal myFather;
    

    public string animalName;
    public string[] namesMale = { "Can", "Sa", "Ed", "Than", "Thi", "Nhi" };
    private string[] namesFemale = { "Xin", "Yi", "Kay", "Lie", "Hay", "Jelly" };
    ...
    IEnumerator spawnChild(Animal male, Animal female)
    {
        ...
        int childCounter = Random.Range(minChildren, maxChildren); //A random integer between 1 and 3 that specifies the number of children to spawn.
        yield return new WaitForSeconds(5f);

        GetComponent<Movement>().agent.isStopped = true;
        for (int i = 1; i <= childCounter; i++) //The corresponding number of children will be spawned.
        {
    ...
    //If the method is set to "true", the corpse disappears after the death animation has finished.
    //Otherwise, it remains lying down (e.g., if it is still to be eaten) and loses one nutrient point per second.
